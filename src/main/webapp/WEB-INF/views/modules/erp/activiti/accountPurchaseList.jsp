...
<body>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="accountPurchase">
			<tr>
				<td><a href="${ctx}/erp/accountPurchase/form?id=${accountPurchase.id}">
					<fmt:formatDate value="${accountPurchase.updateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</a></td>
				<td>
					${accountPurchase.remarks}
				</td>
				<shiro:hasPermission name="erp:accountPurchase:edit"><td>
    				<a href="${ctx}/erp/accountPurchase/form?id=${accountPurchase.id}">修改</a>
					<a href="${ctx}/erp/accountPurchase/delete?id=${accountPurchase.id}" onclick="return confirmx('确认要删除该订货单吗？', this.href)">删除</a>
