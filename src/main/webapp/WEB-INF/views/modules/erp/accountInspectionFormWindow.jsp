	...
	$(document).on("click","#submitbtn",function(){
		...
		$("#contentTable>tbody>tr").each(function(){
			...
			if($(this).children("td:first").children(":checkbox").is(":checked")){
				...
				var unitprice=$(this).children("td:eq(4)").text().trim();
				var suppliercode=$(this).children("td:eq(5)").text().trim();
				var materialcode=$(this).children("td:eq(6)").text().trim();	
				//alert(supplier);
				...
				html+='<tr id="accountPurchaseDetailList'+i+'">'
							...
							+'</td>'
							+'<td>'
							+'	<select id="accountPurchaseDetailList'+i+'_packway" name="accountPurchaseDetailList['+i+'].packway" data-value="" class="input-small ">'
							+'		<option value=""></option>'
							+'		<option value="0">密封</option>'
							+'		<option value="1">半密封</option>	'
							+'	</select>'
							+'</td>'
							+'<td>'
							+'	<select id="accountPurchaseDetailList'+i+'_transport" name="accountPurchaseDetailList['+i+'].transport" data-value="" class="input-small ">'
							+'		<option value=""></option>'
							...
							+'</td>'
							+'<td>'
							+'	<input id="accountPurchaseDetailList'+i+'_freightfee" name="accountPurchaseDetailList['+i+'].freightfee" type="text" value="" class="input-small ">'
							+'</td>'
							+'<td>'
							+'	<input id="accountPurchaseDetailList'+i+'_unitprice" readonly name="accountPurchaseDetailList['+i+'].unitprice" value="'+unitprice+'" type="text" value="" class="input-small ">'
							+'</td>'
							+'<td>'
							+'	<input id="accountPurchaseDetailList'+i+'_quantity" name="accountPurchaseDetailList['+i+'].quantity" type="text" value="" maxlength="11" class="input-small required">'
							+'</td>'
							+'<td>'
							+'	<input id="accountPurchaseDetailList'+i+'_receivedate" name="accountPurchaseDetailList['+i+'].receivedate" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate " value="" onclick="WdatePicker({dateFmt:\'yyyy-MM-dd HH:mm:ss\',isShowClear:false});">'
							+'</td>'
							+'<td>'
							+'	<input id="accountPurchaseDetailList'+i+'_totlemoney" name="accountPurchaseDetailList['+i+'].totlemoney" type="text" value="" class="input-small ">'
							+'</td>'
							+'<td>'
							...
							if( $.inArray(supplier,supplierList)<0){
								supplierListhtml+='<tr id="supplierList'+i+'">'
								+'<td>'
								+'<input id="supplierList'+i+'_supplier" name="supplierList['+i+'].supplier"  readonly type="text" value="'+supplier+'" maxlength="35" class="input-small required"/>'
								+'</td>'
								+'<td>'
								+'	<input id="supplierList'+i+'_deadline" name="supplierList['+i+'].deadline" type="text" readonly="readonly" maxlength="20" class="input-medium Wdate " value="" onclick="WdatePicker({dateFmt:\'yyyy-MM-dd HH:mm:ss\',isShowClear:false});">'
								+'</td>'
								+'<td>'
								+'	<select id="supplierList'+i+'_payway" name="supplierList['+i+'].payway" data-value="" class="input-small ">'
								+'		<option value=""></option>'
