...
<body>
 ...
 <%-- 	<form id="searchForm" action="${ctx}/act/task/process/" method="post" class="breadcrumb form-search">
	...
	<%-- <c:forEach items="${page.list}" var="object">
				...
				<div style="width:100px;height:100px;border:1px solid;float:left;margin-left:5px;" >
					<a href="${ctx}/act/task/form?procDefId=${process.id}">新建${process.name}</a>
				</div>		
	</c:forEach> --%>
	
	<div style="text-align:center;width:110px;height:130px;float:left;background:url(/jeesite/img/apply.png) no-repeat 5px 5px;" >
		<a style="width:110px;height:130px;display:table-cell;vertical-align:bottom;" href="${ctx}/erp/accountRequisition/form">采购申请</a>
	</div>
	
	
	<div style="width:110px;height:130px;text-align:center;float:left;margin-left:5px;background:url(/jeesite/img/verify.png) no-repeat 5px 5px;">
		<a style="width:110px;height:130px;display:table-cell;vertical-align:bottom;" href="${ctx}/erp/accountPurchase/form">订货申请</a>
	</div>
		...
		<tbody>
			<c:forEach items="${page.list}" var="object">
				...
				<tr>
					...
					<td>
						<a href="${ctx}/act/task/form?procDefId=${process.id}">新建流程</a>
					</td>
				</tr>
			</c:forEach>
		</tbody>
