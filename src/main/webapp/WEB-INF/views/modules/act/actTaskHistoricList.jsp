	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="act">
			<tr>
				<td>${act.title}</td>
				<td>
					<c:if test="${act.actindex==0}">
    						采购申请
					</c:if>
					<c:if test="${act.actindex==1}">
    						订货申请
					</c:if>
				</td>
				<td><fmt:formatDate value="${act.start_time}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td>
					<c:if test="${act.actindex==0}">
    					<a href="${ctx}/erp/accountRequisition/formView?id=${act.id}">查看</a>	
					</c:if>
					<c:if test="${act.actindex==1}">
    					<a href="${ctx}/erp/accountPurchase/formView?id=${act.id}">查看</a>	
					</c:if>
					<c:if test="${act.proc_ins_id=='start'}">
						<c:if test="${act.actindex==0}">
    						<a href="${ctx}/erp/accountRequisition/revoke?id=${act.id}">撤销</a>	
						</c:if>
						<c:if test="${act.actindex==1}">
    						<a href="${ctx}/erp/accountPurchase/revoke?id=${act.id}">撤销</a>	
						</c:if>	
					</c:if>
